#!/usr/bin/env bash
# Flash then immediately open the serial monitor (idf.py's combined mode).
# Same USB-attach prerequisite as scripts/flash.sh. Exit monitor with Ctrl+] .
# Run from WSL: ./scripts/flash_monitor.sh [/dev/ttyACM0]
set -euo pipefail
cd "$(dirname "$0")/.."
