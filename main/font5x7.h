...
/*
 * Classic fixed-space 5x7 bitmap font, vendored from Adafruit_GFX
 * (glcdfont.c, BSD-licensed, https://github.com/adafruit/Adafruit-GFX-Library)
 * since it's the well-tested standard rather than a hand-transcribed table.
 * Platform-specific PROGMEM/AVR/ESP8266 wrapping stripped since ESP32-S3
 * needs none of it -- a plain `static const` array already lives in flash.
 *
 * Layout: 5 bytes per glyph, indexed by ASCII code (0-255). Each byte is one
 * column, bit 0 = top pixel, bits 0-6 used (7 rows visible).
 */
